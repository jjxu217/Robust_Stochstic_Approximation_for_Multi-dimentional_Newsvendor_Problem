    ...
    for(r = 0; r < config.NUM_REPS;r++){
        ...
        for(n = 1; n < config.N1; n++){
            ...
            *F^-1(x) = ((1-x)^(-1/beta)-1)^(1/alpha)*/
            ...
            solveSub(x, lambda, &config.RUN_SEED[r]);
            for(i = 0; i < NUMofRESOURCE; i++){
                x[i] = x[i] - config.STEP_SIZE * (5 + lambda[i]);
                if(x[i] < 0){
                    x[i] = 0;
                }
                if(x[i] > 5){
...
int readConfig() {
    ...
    while ((status = (fscanf(fptr, "%s", line) != EOF))) {
        ...
        else if (!(strcmp(line, "STEP_SIZE")))
            fscanf(fptr, "%lf", &config.STEP_SIZE);
